.PHONY: start-worker execute installdeps start-examples execute-insurance-claims execute-cargo-release execute-code-modernization execute-erp-health
...
## Execute the erp-health-report workflow
## Usage: make execute-erp-health input='{"odoo_url": "...", "odoo_db": "...", "odoo_username": "...", "odoo_password": "...", "email_to": "...", "output_dir": "./reports"}'
execute-erp-health:
	uv run python -m entrypoints.start --workflow erp-health-report $(if $(input),--input '$(input)',--input '{"odoo_url":"http://localhost:8069","odoo_db":"odoo","odoo_username":"admin","odoo_password":"admin","email_to":"admin@example.com","output_dir":"./reports"}')
